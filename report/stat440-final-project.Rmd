...
\begin{itemize}
\item Implemented parameter inferences for BM and OU models and tested likelihoods and estimates by unit tests
...
Compared to BM model, the OU process includes more factors from the environment that can impact the change of the distance. The OU process has four components to describe the molecule interactions: $\gamma$, $\mu$, $\sigma$, and $dW_t$.The interpretation of $\gamma$ is the rate of mean reversion. The OU process sharesthe  mean reversion nature with the BM. The rate indicates how strong the distance will react toward the attractor. $\mu$ is the asymptotic mean of "bond" length. $\sigma$ is the parameter showing the volatility of noise[@oubio16]. The OU process represents how molecule move towards the attractors $\mu$ with BM. 
