...
test_that("OU theta hat relative error falls within tolerance ",{
  test_cases <- 1:4
  ...
  ntheta <- 2 # number of parameter sets per test case
  ...
  ou_test_out <- sapply(test_cases, function(ii) {
    ...
    nll_diff <- replicate(ntheta, expr = {
      ...
      dproj <- diff(oproj)
      expect_equal(dproj[,c('rel')][["mu"]], 0, tol=tol)
      expect_equal(dproj[,c('rel')][["omega"]], 0, tol=tol)
      expect_equal(dproj[,c('rel')][["tau"]], 0, tol=tol)
    })
  })
  ...
  bm_test_out <- sapply(test_cases, function(ii) {
    ...
    nll_diff <- replicate(ntheta, expr = {
      ...
      expect_equal(dproj[,c('rel')], 0, tol=tol)
    })
  })
})
