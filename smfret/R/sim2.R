...
sim_2 <- function(from, beta0 = 20, beta1 = .5, gamma = 1, mu = 10, sigma=0.1, dt = 1,
    ...
    models <- replicate(n_dataset, expr = {
        ...
        while (is.na(ou_aic) || is.na(bm_aic)) {
            ...
            print("reach3")
            ou_f <- TMB::MakeADFun(data = data, parameters = param, random = c("X"), silent = TRUE, method=method)
            print("reach4")
            ...
            bm_f <- TMB::MakeADFun(data = data, parameters = param, random = c("X"), silent = TRUE, method=method)
